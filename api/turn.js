...
if(req.method==='GET')return reply(res,200,{configured,mode:'open-world',version:3,build:(env.VERCEL_GIT_COMMIT_SHA||'local').slice(0,12)});
...
if(body.start===true){try{const state=world.initial(typeof body.name==='string'?body.name:'Rowan',body.cls);return reply(res,200,{state,save:world.sign(state,secret)})}catch{return reply(res,400,{error:'Choose a character class.'})}}
...
try{const plan=await groq.generate(world.planMessages(old,action),world.planSchema,{env,stage:'plan'});const {state,resolution}=world.resolve(old,plan,undefined,action);const result=await groq.generate(world.narrateMessages(state,action,resolution),world.turnSchema,{env,stage:'narrate'});const updated=world.apply(state,result,action,resolution);const data={state:updated,save:world.sign(updated,secret),resolution};responses.set(cacheKey,{data,expires:Date.now()+120000});return reply(res,200,data)}catch(e){const provider=e instanceof groq.ProviderError;if(provider)logProviderError(e);else console.error('[astra-dnd turn]',e instanceof Error?e.message:String(e));const status=provider?e.status:503;const retryAfter=provider?e.retryAfter||0:0;if(retryAfter)res.setHeader('Retry-After',String(retryAfter));return reply(res,status,{error:provider?e.message:'The turn could not be safely resolved. Your save is unchanged; please try again.',retryAfter})}finally{inflight.delete(old.id)}
};

...
try{const plan=await groq.generate(world.planMessages(old,action),world.planSchema,{env});const {state,resolution}=world.resolve(old,plan);const result=await groq.generate(world.narrateMessages(state,action,resolution),world.turnSchema,{env});const updated=world.apply(state,result,action,resolution);const data={state:updated,save:world.sign(updated,secret),resolution};responses.set(cacheKey,{data,expires:Date.now()+120000});return reply(res,200,data)}catch(e){const status=e instanceof groq.ProviderError?e.status:503;const retryAfter=e.retryAfter||0;if(retryAfter)res.setHeader('Retry-After',String(retryAfter));return reply(res,status,{error:e instanceof groq.ProviderError?e.message:'The turn could not be safely resolved. Your save is unchanged; please try again.',retryAfter})}finally{inflight.delete(old.id)}
};
